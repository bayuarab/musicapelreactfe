...
import { Alert, Box, Card, CardActionArea, CardContent, CardMedia, FormControl, Grid, IconButton, InputLabel, MenuItem, Select, Snackbar, Stack, Typography } from "@mui/material";
...
export default function CategoryCourse() {
	...
	const fetchDelete = async () => {
		try {
			...
			const response = await api.delete(`/Cart/${auth.userId}/${idCart}`, config);
					...
					{claimedCourse ? (
						...
						<>
							...
							<Box
								...
								<StyledPurpleButton variant="contained" component={Link} to={`/cart`}>
									Ke Cart
								</StyledPurpleButton>
								<StyledPurpleButton variant="contained" sx={{ marginLeft: "3%" }} onClick={() => checkout()}>
									Beli Sekarang
								</StyledPurpleButton>
							</Box>
						</>
