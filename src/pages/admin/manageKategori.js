...
import { Box, Button, Card, CardActionArea, CardActions, CardContent, Container, Grid, IconButton, Input, Paper, TextField, Toolbar, Typography } from "@mui/material";
	...
	const handleCloseDelete = (state) => {
		...
		const fetchDelete = async () => {
			try {
				...
				if (err.response.status === 401 || err.response.status === 403) setErr("Otoritas tidak berlaku silahkan login kembali");
				setOpen(true);
			}
		};
		fetchDelete();
				...
				<Grid>
					<Card>
						...
						<CardActions style={{ backgroundColor: "", justifyContent: "center" }}>
							...
							{/* Delete button */}
							<Button startIcon={<DeleteForever />} variant="outlined" size="medium" color="remove" style={{ backgroundColor: "F2C94C", fontFamily: "Poppins", textTransform: "capitalize" }} onClick={() => handleClickOpenDelete(item)}>
								Hapus
							</Button>
						</CardActions>
					</Card>
