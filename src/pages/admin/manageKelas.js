import { AddCircle, DeleteForever, List, ModeEdit, Search } from "@mui/icons-material";
import { Alert, Box, Button, CardContent, CardMedia, Container, Grid, IconButton, Paper, Snackbar, Stack, TextField, Toolbar, Tooltip, Typography, Zoom } from "@mui/material";
...
const ReadMore = ({ children }) => {
	...
	return (
		<Typography className="text">
			...
			<span style={{ color: "#5D5FEF", fontWeight: "500", fontSize: "15px", fontFamily: "Poppins" }} onClick={toggleReadMore} className="read-or-hide">
...
function ManageKelas() {
	...
	const getListOfBrands = async () => {
		await apiBase
			...
			.catch((err) => {
				...
				if (err.status === 401 || err.status === 403) setLoadMessage("Otoritas tidak berlaku silahkan login kembali");
			});
	};
	...
	const kategoriFilter = () => {
		return searchQuery?.length > 0 ? listOfBrands?.filter((item) => item.category.includes(searchQuery) || item.id.toString().includes(searchQuery)) : listOfBrands;
	};
									...
									<Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
										...
										{kategoriFilter().map((item) => (
											<Grid key={item.id} style={{ margin: "2% 0" }}>
												...
												<Box sx={{ height: "0px", borderBottom: "0.5px solid gray" }}></Box>
											</Grid>
										))}
									</Paper>
								</Grid>
