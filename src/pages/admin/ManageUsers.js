...
function ManageUser() {
	...
	const handleCloseLogout = (state) => {
		...
		const fetchDelete = async () => {
			try {
				...
				if (response?.data) {
					setUsers((item) => item.filter((item) => item.email !== selectedUser.email));
				}
				...
				if (err.response.status === 401 || err.response.status === 403) setMessage("Otoritas tidak berlaku silahkan login kembali");
				setSnackbarState(true);
			}
		};
		fetchDelete();
	...
	const userFilter = () => {
		return searchUser?.length > 0 ? users?.filter((item) => item.nama.includes(searchUser) || item.email.includes(searchUser)) : users;
	};
