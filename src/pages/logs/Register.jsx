...
export default function Login() {
	...
	const handleSubmit = (event) => {
		...
		const vNama = NAME_REGEX.test(nama);
		...
		if (!vPass) {
			setErr("Password harus terdiri dari 8-24 karakter dan harus terdapat uppercase, lowercase, serta angka");
			setOpen(true);
			return;
		}
		...
		const fetchApi = async () => {
			try {
				await api.post("/UserAuth/Register", dataClient);
